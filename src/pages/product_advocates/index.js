...
const Batches = () => {
    ...
    const fetchTableData = useCallback(
        async (sort, column) => {
            await axios
                .get(BASE_URL + '/product_advocate/find_all', {
                    headers: {
    ...
    const handleUpdateAdvocate = async (e, advocateId, companyId) => {
        try {
            const response = await axios.post(
                BASE_URL + '/product_advocate/update_company',
                {
                    recordId: advocateId,
    ...
    const columns = [
        ...
        },
        {
            flex: 0.2,
            minWidth: 130,
            field: "Assign Company",
            headerName: "Assign Company",
            renderCell: ({ row }) => {
                return (
                    <Grid item xs={12} sm={12}>
                        <FormControl fullWidth variant="standard" sx={{ border: "none" }}>
                            <Select
                                ...
                                onChange={(e) => {
                                    handleUpdateAdvocate(e, row?.Id, e.target.value);
                                }}

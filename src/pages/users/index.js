 ...
 const fetchCompanies=async()=>{
  try{
    ...
    if(userData.role=="Auth_Admin"){
      ...
      const commonElements = applicationsData2.filter(item1 => user.access.some(item2 => item2.application.id === item1.id));
 ...
 useEffect(()=>{
  ...
  const handleSubmit=async(e)=>{
    ...
    try{
      ...
      await axios.post(BASE_URL+`/users/CreateUserBySuperAdmin/${state.selectedCompany}`,{
        email: state.email,
    ...
    await axios.post(BASE_URL+`/users/CreateUserByAdmin?${state.selectedCompany}`,{
      email: state.email,
      userName: state.username,
      password: state.password,
      access: result.access
  ...
  return (
    <>
        ...
        <DialogContent>
          <CardContent>
            <Grid container>
              ...
              <TextField  required type='text' value={state.username} onChange={(e)=>setState({...state, username:e.target.value})} id="standard-basic" name="username" label="Username" placeholder='Enter username' variant="standard" />
            </FormControl>
            <FormControl sx={{ width:400, mb:6}}>
              <TextField  required type='email' value={state.email} onChange={(e)=>setState({...state, email:e.target.value})} id="standard-basic" name="email" label="Email" placeholder='Enter email address' variant="standard" />
            </FormControl>
            <FormControl sx={{ width:400, mb:6}}>
              <TextField  required type='password' value={state.password} onChange={(e)=>setState({...state, password:e.target.value})} id="standard-basic" name="password" label="Set Password" placeholder='Set password for user' variant="standard" />
            </FormControl>
      ...
      <Button variant="contained" onClick={handleAddAssignment} disabled={!selectedApp || !selectedRole}>
        Assign Access
      </Button>
      <List>
